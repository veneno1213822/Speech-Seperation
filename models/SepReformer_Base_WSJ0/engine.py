...
class Engine(object):
    ...
    @logger_wraps()
    def _inference_sample(self, sample, output):
        self.model.eval()
        ...
        with torch.inference_mode():
            ...
            estim_src, _ = torch.nn.parallel.data_parallel(self.model, nnet_input, device_ids=self.gpuid)
            mixture = torch.squeeze(mixture).cpu().data.numpy()
            sf.write(output+sample.split('/')[-1][:-4]+'_in.wav', 0.9*mixture/max(abs(mixture)), self.fs)
            for i in range(self.config['model']['num_spks']):
                src = torch.squeeze(estim_src[i][...,:mixture.shape[-1]]).cpu().data.numpy()
                sf.write(output+sample.split('/')[-1][:-4]+'_out_'+str(i)+'.wav', 0.9*src/max(abs(src)), self.fs)

    @logger_wraps()
    def _inference_sample_multi(self, sample, output):
        self.model.eval()
        self.fs = self.config["dataset"]["sampling_rate"]
        mixture, _ = librosa.load(sample, sr=self.fs)
        mixture = torch.tensor(mixture)[None]
        ...
        with torch.inference_mode():
            ...
            for i in range(self.config['model']['num_spks']):
                src = torch.squeeze(estim_src[i][..., :mixture.shape[-1]]).cpu().data.numpy()
                sf.write(output+f's{i+1}_out/'+sample.split('/')[-1][:-4]+'_out_'+str(i)+'.wav',0.9*src/max(abs(src)),self.fs)
