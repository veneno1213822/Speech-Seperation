...
class PIT_SDRi:
    ...
    #     input = torch.cat([kwargs['mixture'], kwargs['mixture']], dim=0)
    #
    #     # 转换为 NumPy 数组的部分改为 PyTorch 张量操作
    #     targets_cpu = targets.cpu()  # 直接保持在 PyTorch 中，不转换为 numpy
    #     estims_cpu = estims.cpu()
    #     input_cpu = input.cpu()
    #
    #     # 使用 bss_eval_sources 函数并确保其输出适合 PyTorch 操作
    #     min_perutt_out, _, _, _ = bss_eval_sources(targets_cpu, estims_cpu)
    #     min_perutt_in, _, _, _ = bss_eval_sources(targets_cpu, input_cpu)
    #
    #     # 使用 PyTorch 的 sum 操作，计算损失
    #     num_utts = input_sizes.shape[0]
    #     loss = torch.sum(min_perutt_out - min_perutt_in) / num_utts
    #     return loss, min_perutt_out - min_perutt_in
    #     # targets = targets.cpu().detach().numpy()  # targets = targets.cpu().data.numpy()
    #     # estims = estims.cpu().detach().numpy()  # estims = estims.cpu().data.numpy()
    #     # input = input.cpu().detach().numpy()  # input = input.cpu().data.numpy()
    #     #
    #     # min_perutt_out, _, _, _ = bss_eval_sources(targets, estims)
    #     # min_perutt_in, _, _, _ = bss_eval_sources(targets, input)
    #     #
